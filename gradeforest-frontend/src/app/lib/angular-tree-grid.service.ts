...
export class AngularTreeGridService {
  ...
  findRowIndex(
    display_data: any[],
    configs: { id_field: string | number },
    row_id: any
  ) {
    return display_data.map((row) => row[configs.id_field]).indexOf(row_id);
  }
  ...
  selectOne(display_data: any[], configs: DefaultConfigs, row_id: any) {
    display_data.forEach((data) => {
      if (data[configs.id_field] == row_id) {
        data.row_selected = true;
      } else {
        data.row_selected = false;
      }
  ...
  expandRow(
    row_id: any,
    expand_tracker: { [x: string]: boolean },
    expand_event: EventEmitter<any>,
    suppress_event: boolean | undefined,
  ...
  collapseRow(
    row_id: any,
    expand_tracker: any,
    collapse_event: EventEmitter<any>,
    suppress_event: boolean | undefined,
    configs: DefaultConfigs,
    display_data: any[]
  ) {
  ...
  disableRowExpand(display_data: any[], configs: DefaultConfigs, row_id: any) {
    const row_index = this.findRowIndex(display_data, configs, row_id);
    display_data[row_index].expand_disabled = true;
  }

  enableRowExpand(display_data: any[], configs: DefaultConfigs, row_id: any) {
    const row_index = this.findRowIndex(display_data, configs, row_id);
  ...
  emitSubgridExpandRowEvent(
    ...
    promise
      .then((child_rows: any) => {
        ...
        if (child_rows) {
          child_rows.map((child: { leaf: boolean }) => {
            child.leaf = true;
          });
