...
export class Store {
   ...
   * @returns blank_row
   */
  showBlankRow(row_data: { [x: string]: any }) {
    const row_index = this.display_data
  ...
  remove_children(row_data: { [x: string]: any }) {
  ...
  add_children(row_data: { [x: string]: any }, children: any) {
    const row_index = this.processed_data
      .map((row) => row[this.configs.id_field])
      .indexOf(row_data[this.configs.id_field]);
  ...
  filterBy(columns: string | any[], search_values: any[]) {
  ...
  findTopParentNode(data: any[], configs: DefaultConfigs) {
  ...
  processData(
    ...
    configs: Configs,
    edit_tracker: { [x: string]: boolean },
    internal_configs: { top_parents: any[] }
  ) {
  ...
  findChildren(data: any[], id: any, configs: DefaultConfigs) {
